...
export const useSaveTracking = (options: SaveTrackingOptions = {}) => {
	...
	const { 
		...
		const hasBlock = blocks.some((block: any) => 
			block.name === blockName || 
			block.innerBlocks?.some((inner: any) => inner.name === blockName)
		);
	...
	useEffect(() => {
		...
		if (!isSaving && previousIsSaving.current && didSaveSucceed && !isAutosaving && hasSliderbergBlock) {
	...
	}, [isSaving, isAutosaving, didSaveSucceed, hasSliderbergBlock, onSaveComplete]);
