...
function updateSliderbergSlidesVisibility(): void {
  ...
  document.querySelectorAll<HTMLElement>('.sliderberg-posts-slider-wrapper').forEach(wrapper => {
    const postSlides = Array.from(wrapper.querySelectorAll<HTMLElement>('.sliderberg-post-slide'));
    
    if (postSlides.length > 0) {
      // Check if any slide is already marked as active
      const activeSlide = postSlides.find(slide => 
      ...
      if (activeSlide) {
        ...
        postSlides.forEach((slide) => {
          const isActive = slide.getAttribute('data-is-active') === 'true';
  ...
  document.querySelectorAll<HTMLElement>('.sliderberg-woo-products-wrapper').forEach(wrapper => {
    const productSlides = Array.from(wrapper.querySelectorAll<HTMLElement>('.sliderberg-product-slide'));
...
const observer: MutationObserver = new MutationObserver(() => {
  ...
  (window as any).visibilityUpdateTimeout = setTimeout(updateSliderbergSlidesVisibility, 100);
});
